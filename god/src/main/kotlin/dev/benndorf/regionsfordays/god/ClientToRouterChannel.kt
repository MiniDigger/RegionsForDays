...
class ClientToRouterChannel(private val clientPlayer: ClientPlayer, private val router: Router) : EventHandler {
  override fun sendEvent(uuid: UUID, event: Event) {
    ...
    router.incomming(router.findClient(clientPlayer.uuid) ?: RouterPlayer(clientPlayer.uuid, clientPlayer.name, clientPlayer.pos, clientPlayer.areaOfInterest), event, this)
  }
